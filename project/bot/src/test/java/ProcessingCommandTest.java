...
public class ProcessingCommandTest {
    ...
    public void testEmptyLinksList() {
        Assert.assertEquals("Список отслеживаемых ссылок пуст", new ListCommandProcessingUnit(new ArrayList<>()).processCommand("/list"));
    }
    ...
    public void testUnknownCommand() {
        ...
        Mockito.when(mock.processCommand("/start")).thenReturn("sorry, feature \"user registration\" is under construction");
        Assert.assertEquals("Извините, данная команда не поддерживается", new TelegramBotService(List.of(mock)).processMessage(unknownCommand));
    }
    ...
    public void testCorrectCommands() {
        ...
        Assert.assertEquals("sorry, feature \"track\" is under construction", new TelegramBotService(List.of(mock)).processMessage("/track"));
    }
}
